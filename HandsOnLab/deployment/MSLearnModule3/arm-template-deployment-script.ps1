...
gridingstepmodelid=$(az dt model create -n $dtname --models /mnt/azscripts/azscriptinput/digital-twins-samples/HandsOnLab/models/ProductionStepGrinding.json --query [].id -o tsv)
  ...
  result="{\"PS-Script\":"\"" start-copy-> \$rgname = '$rgname';\$location = '$location';\$dtname = '$dtname';\$functionstorage = '${prefix}storage';\$telemetryfunctionname = '${prefix}-telemetryfunction';\$twinupdatefunctionname = '${prefix}-twinupdatefunction'; \$username = 'replaceme@contoso.com' <-endcopy "\""}"
